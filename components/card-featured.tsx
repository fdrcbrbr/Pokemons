  ...
  return (
    <CardWrapper>
      {featuredPokemons.map((pokemon) => (
        ...
        >
          <div
            className={`w-32 h-32 rounded-full border-4 ${getColorFromType(
              pokemon.type[0],
              'border-[{color}]'
          ...
          <div className="mt-2 text-center">
            <p
              className={`inline-block px-2 text-gray-600 text-sm rounded-full mt-2 ${getColorFromType(
                pokemon.type[0],
                'bg-[{color}]'
              )}`}
            >
            ...
            <div className="flex flex-wrap gap-2">
              {pokemon.type.map((type, index) => (
                <span
                  key={index}
                  className={`inline-block px-3 py-1 text-xs font-semibold text-white rounded-full mt-2 ${getColorFromType(
                    type,
                    'bg-[{color}]'
                  )}`}
                >
